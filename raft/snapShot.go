...
func NewSnapShotMetaData(index, term int) SnapShotMetaData {
...
func NewSnapShot(index, term int, data []byte) *SnapShot {
	return &SnapShot{
...
// SaveSnap 存储并持久化快照
func (s *SnapShotManager) SaveSnap(shot *SnapShot) error {
	if shot == nil || shot.empty() {
		return errors.New("snap is empty")
	}
	return s.save(shot)
}
