...
func NewHTTPServer(id int, endpoint string, s *raft.Server) *HTTPServer {
	// 创建 Gin 引擎
	hs := new(HTTPServer)
	hs.id = id
...
func (hs *HTTPServer) Get(key string, reply *raft.CommandReply) error {
	args := raft.CommandArgs{
		Op:        raft.GET,
		Key:       key,
...
func (hs *HTTPServer) Put(key, value string, reply *raft.CommandReply) error {
	args := raft.CommandArgs{
		Op:        raft.PUT,
		Key:       key,
		Value:     value,
...
func (hs *HTTPServer) Delete(key string, reply *raft.CommandReply) error {
	args := raft.CommandArgs{
		Op:        raft.DELETE,
		Key:       key,
		CommandId: idGenerator.GenerateID(),
...
func (hs *HTTPServer) handleCommand(args raft.CommandArgs, reply *raft.CommandReply) error {
	hs.mu.Lock()
	...
	for i := 0; i < 3; i++ {
		...
		case raft.WRONG_LEADER: // 更换leader
			if reply.LeaderId < 0 {
				common.SleepMs(500)
			} else {
